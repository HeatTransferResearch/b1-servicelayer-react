I developed this application with npm 3.10.10, and node 6.11.0 on Ubuntu 16.04.2 LTS.
 
Prerequisites:
- add `"CorsEnable": true` and `"CorsAllowedOrigns": "http://localhost:3000"` to your service layer configuration file (b1s.conf) on the HANA appliance.
- ensure that your browser will accept the certificate from the service layer by visiting https://thehanahostname:50000/

In your development environment: 
- install npm and node 
- update `src/ServiceLayer.js` so that it refers to your HANA appliance hostname.  
- update `src/ServiceLayerLogin.js` and change the CompanyDB from SBOSL to a company schema name you wish to use.
- run `npm install` to download the dependencies
- run `npm start` to run the app
- log in with a pro user
